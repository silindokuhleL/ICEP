<?php
	...
	if(isset($_POST["activate"])) {
		...
		if (isset($_POST["id"])){
			$id = $_POST["id"];
			$status = "Active";
			$result = $adminDAO->updatePatient($con, $id, $status);
			
			if ($result == 1) {
				message("Patient have been activated sucessfully");
			} else {
		...
		if (isset($_POST["id"])) {
			...
			if ($result == 1) {
				message("Patient have been deactivated sucessfully");
			} else {
				message("Something went wrong");
			}
						...
						<div class="panel-heading"><h5 class="thin text-center">All Registered Patients</h5></div>
							<div class="panel-body">
								<form action="<?php echo $_SERVER['PHP_SELF'] ;?>" method="POST">
									<?php
										require_once('dbConnect.php');
										$result = $adminDAO->getAllPatients($con);
										if ($result != null ) {
											if ($result->num_rows > 0) {
												print '<table class="table table-bordered" width="50">
													...
													while($row = $result->fetch_assoc()) {
														...
														$email = $row["email"];
														$status = $row["status"];
														print"<tr class='info'><td><input type='radio' value='$id' name='id'/></td><td>$name</td><td>$lastname</td><td>$cellNumber</td><td>$email</td><td>$status</td></tr>";
											...
											?>
												<div class="row">
													<div class="col-md-6">
														<button class="btn btn-checkout" name="activate" type="submit">Activate Patient</button>
													</div>
													<div class="col-md-6 text-right">
														<button class="btn btn-checkout" name="deactivate" type="submit">Deactivate Patient</button>
													</div>
												</div>
											<?php
											} else {
